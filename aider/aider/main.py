...
def register_litellm_models(git_root, model_metadata_fname, io):
    ...
    try:
        ...
        if len(model_metadata_files_loaded) > 0:
            io.tool_output(
                f"Loaded {len(model_metadata_files_loaded)} model metadata file(s)"
            )
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    result = []  # Create a list to store the output

    if args.message:
        io.add_to_input_history(args.message)
        io.tool_output()
        result = coder.run(with_message=args.message)
    elif args.message_file:
        try:
            message_from_file = io.read_text(args.message_file)
            io.tool_output()
            result = coder.run(with_message=message_from_file)
        except FileNotFoundError:
