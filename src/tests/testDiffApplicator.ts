...
assert.deepStrictEqual(edits, [{ filePath: "foo.txt", originalText: "Two\n", updatedText: "Tooooo\n" }]);
    });

test('should handle mangled filename with source tag', () => {
    const source = "source";
    const edit = `
Here's the change:
...
test('should handle quote below filename', () => {
    const edit = `
Here's the change:
...
test('should handle incomplete edit block with missing filename', () => {
    const edit = `
