...
export class MeltyExtension {
        ...
        try {
            ...
            try {
                ...
                if ((error as any).status === 404) {
                    console.log(
                        "Remote branch does not exist, will create new"
                    );
                } else {
            ...
            try {
                ...
                if ((error as any).status === 404) {
                    console.log(
                        "Commit does not exist on remote, pushing changes"
                    );
            ...
            try {
                ...
                if (
                    ...
                    console.log(
                        "Pull request already exists, fetching existing PR"
                    );
                    try {
                        ...
                        if (pulls.length > 0) {
                            ...
                            vscode.env.openExternal(
                                vscode.Uri.parse(existingPR.html_url)
                            );
                        } else {
                            vscode.window.showErrorMessage(
    ...
    private getOwnerAndRepo(repository: any): [string, string] | null {
        ...
        if (match && match.length === 3) {
            ...
            console.error(
                "Failed to extract owner and repo from URL:",
                remoteUrl
            );
            return null;
        }
