...
class MeltycatService {
	...
	constructor(targetDirectory: string, config: MeltycatConfig) {
		this.watchPath = targetDirectory;
	...
	private async processBatchUpdate(): Promise<void> {
		...
		for (const relativePath of changes) {
			...
			try {
				...
				if (content !== null) {
					this.summaries.set(relativePath, { file: relativePath, content });
	...
	private async updateSummaryFile(): Promise<void> {
		...
		try {
			await fs.promises.writeFile(this.summaryPath, summaryContent, 'utf8');
		} catch (error) {
...
export default MeltycatService;
