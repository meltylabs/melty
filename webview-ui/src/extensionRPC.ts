...
export class ExtensionRPC {
    ...
    public run(method: rpcMethod, params: any = {}): Promise<any> {
        return new Promise((resolve, reject) => {
            ...
            console.log(`[ExtensionRPC] Webview is sending message ${id} with method ${method} with params ${JSON.stringify(params)}`);
    ...
    public handleMessage(event: MessageEvent) {
        ...
        if (message.type === 'rpcResponse') {
            ...
            if (pending) {
                ...
                if (message.error) {
                    console.log(`[ExtensionRPC] rejecting message ${message.id} with error ${message.error}`);
                    pending.reject(message.error);
                } else {
                    console.log(`[ExtensionRPC] resolving message ${message.id} with result ${message.result}`);
                    pending.resolve(message.result);
                }
            } else {
                console.warn(`[ExtensionRPC] received response for unknown message ${message.id}`);
            }
        }
    }
}
